...
namespace soundmath {
	...
	//! Base class for the metatemplate FFT
	template<typename T>
	class AbstractFFT {
	public:
	...
	template <typename T>
	class VFFT : public AbstractFFT<T> {
	public:
		VFFT (int size) {
			m_fftSize = size;
			m_fftSizeOver2 = m_fftSize / 2;
			m_log2n = log2f (m_fftSize); // bins
			m_log2nOver2 = m_log2n / 2;

			m_inReal = new T[m_fftSize];
			m_outReal = new T[m_fftSize];
			m_splitData.realp = new T[m_fftSizeOver2];
			m_splitData.imagp = new T[m_fftSizeOver2];

			m_windowSize = size;
			m_window = new T[m_windowSize];
			memset (m_window, 0, sizeof(T) * m_windowSize);
			vDSP_hann_window (m_window, m_windowSize, vDSP_HANN_NORM);
		...
		void forward (T *buffer,
			T* magnitude,
			T* phase) {

			//convert to split complex format with evens in real and odds in imag
			vDSP_ctoz ((COMPLEX *) m_inReal, 2, &m_splitData, 1, m_fftSizeOver2);

			//calc fft
			vDSP_fft_zrip (m_fftSetup, &m_splitData, 1, m_log2n, FFT_FORWARD);

			m_splitData.imagp[0] = 0.0;

			vDSP_ztoc (&m_splitData, 1, (COMPLEX*) m_inReal, 2, m_fftSizeOver2);
			vDSP_polar (m_inReal, 2, m_outReal, 2, m_fftSizeOver2);
			cblas_scopy (m_fftSizeOver2, m_outReal, 2, magnitude, 1);
			cblas_scopy (m_fftSizeOver2, m_outReal+1, 2, phase, 1);
		}

		void inverse (T *buffer,
			T* magnitude,
			T* phase) {
			cblas_scopy (m_fftSizeOver2, magnitude, 1, m_inReal, 2);
			cblas_scopy (m_fftSizeOver2, phase, 1, m_inReal+1, 2);
			vDSP_rect (m_inReal, 2, m_outReal, 2, m_fftSizeOver2);

			//convert to split complex format with evens in real and odds in imag
			vDSP_ctoz ((COMPLEX *) m_outReal, 2, &m_splitData, 1, m_fftSizeOver2);

			vDSP_fft_zrip (m_fftSetup, &m_splitData, 1, m_log2n, FFT_INVERSE);
			vDSP_ztoc (&m_splitData, 1, (COMPLEX*) m_outReal, 2, m_fftSizeOver2);

			vDSP_vsmul (m_outReal, 1, &m_scale, m_outReal, 1, m_fftSize);
		}
	private:
		int	m_fftSize;
		int m_fftSizeOver2;
	...
	void fft (T *fftBuffer, long fftFrameSize, long sign) {
		...
		for (k = 0, le = 2; k < ceil ((T) log (fftFrameSize) / LOG2); k++) {
			le <<= 1;
			le2 = le >> 1;
			ur = 1.;
			ui = 0.;
	...
	void rect2pol (T* cbuf, int N) {
		for (int i = 0; i < N; ++i) {
			T real = cbuf[2 * i];
			T imag = cbuf[2 * i + 1];
			T amp = sqrt (real * real + imag * imag);
	...
	inline void ampFreqPhaseDiff (const T* cbuffer, T* amp, T* freq, T* oldPhi,
		...
		for (int i = 0; i < N; ++i) {
			...
			T phase = atan2 (cbuffer[2 * i + 1], cbuffer[2 * i]);
