...
// int readSamples (const char* name, float* proto, int bsize) {
// 	WavInFile in (name);
...
// 	proto = new float[sz + bsize];
// 	memset (proto, 0, sizeof (float) * (sz + bsize));
...
// 				proto[(i + pointer)] += (tmp1[nchnls * (i + pointer) + j] / nchnls);
// 			}
// 		}
// 		pointer += bsize;
// 	}
